"""
Generate the SLM phase hologram for few-mode fiber mode encoding.

The coefficient section maps each source image/message to an 8-bit modal
coefficient code. The synthesized modal field is then converted into an SLM
phase pattern.
"""
...
def synthesize_field(coeff, modes_ref, H, W):
    ...
    E_out : torch.Tensor
        (B, H, W) 复数光场
    """
    B, N_mode = coeff.shape

    # 叠加模式
    E_flat = torch.matmul(coeff, modes_ref.T)  # (B, H*W)
    E_out = E_flat.reshape(H, W)           # 重塑为 (B,H,W)
...
#%% 定义期望生成的分布
# Image/message-to-mode encoding used in the manuscript.
# Each 8-bit string maps to the amplitudes of the first 8 modes.
# COEFFS_BY_IMAGE_NAME = {
#     "hua-1.PNG": "01010011",
#     "hua-2.PNG": "01001110",
#     "1911-1.PNG": "00000111",
#     "1911-2.PNG": "01110111",
...
#%% 画出目标光场
data= torch.abs(target.detach().cpu())**2
...
plt.show()
#%% 生成棋盘调制相位
L=torch.tensor(100, dtype = torch.float, device = device) #传播距离远一点滤除杂光，或者在4f焦面上操作一下滤波
Lambda = torch.tensor(Lambda,dtype = torch.float, device = device)
k_tensor = 2 * torch.pi/Lambda
SLMdesign = fib.SLMSimulator(Nx=Nx, Ny=Ny, dx=dx, wavelength=Lambda,
                             Et = target, device=device)
SLMpattern, E_m= SLMdesign.simulate(beam_radius = beam_radius, X = Xi, Y = Yi, Ein = None, Shining=True, 
                                    period_pixels = 10, show = True)
Eprop_FFT = ut.FreeSpacePropagrationFFT(E_m, Xi, Yi, L, k_tensor, Lambda, numpad=10, device = device)
#%% 光场追迹
ut.figComplexField(Xi, Yi, torch.abs(Eprop_FFT)**2, 
                    0, string='E_out', cmaps = ['inferno', 'twilight']) #输出场
#%% 保存相位图
ut.save_phase_as_bmp(E_m, filename="fewmode_grating_3mm.bmp")
